...
#include "SimpleMath.h"                /* Model header file */
...
void rt_OneStep(void)
{
  ...
  /* Check for overrun */
  if (OverrunFlag) {
    rtmSetErrorStatus(SimpleMath_M, "Overrun");
    return;
  }
...
int_T main(int_T argc, const char *argv[])
{
  ...
  fflush((NULL));
  while (rtmGetErrorStatus(SimpleMath_M) == (NULL)) {
    /*  Perform application tasks here */
  }

  /* Terminate model */
  SimpleMath_terminate();
  return 0;
}
